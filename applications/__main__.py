...
def newcase(nom_entreprise: str):
    ...
    fichier.close()
    print(
        f"Le fichier modèle (avec les gains à zéro) de {nom_entreprise} a été généré. Vous pouvez le modifier avec les données du réelles"
    )
...
def addweeks(nom_entreprise: str):
    if os.path.exists(nom_entreprise + ".json"):
        ...
        fichier.close()
        print(
            f"Les nouvelles tâches ont été ajoutées à {nom_entreprise} avec les valeurs par défauts (0) à modifier!"
        )
    else:
        print(Message)
